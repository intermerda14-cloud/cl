...
async function handleChart(req, res, db) {
    ...
    if (symbol) {
        chartData = await db.collection('charts').findOne({ _id: symbol });
    }
    ...
    if (chartData) {
        ...
        const symbolData = await db.collection('symbols').findOne({ _id: chartData.symbol || symbol });
        
        return res.status(200).json({
            status: 'ok',
            symbol: chartData.symbol || chartData._id,
