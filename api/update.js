// /api/update.js - Main data endpoint from EA
...
export default function handler(req, res) {
    ...
    if (req.method === 'GET') {
        return res.status(200).json({
            ...
            positions_count: bbData.positions.length,
    ...
    if (req.method === 'POST') {
        try {
            ...
            // Add time and ensure all data fields
            data.last_update = Date.now();
            data.server_time = new Date().toISOString();
            
            // Ensure broker info is preserved
            data.broker = data.broker || data.account_company || '';
            ...
            if (data.positions && Array.isArray(data.positions)) {
                bbData.positions = data.positions;
            }
